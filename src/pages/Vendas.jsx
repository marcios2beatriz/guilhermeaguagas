...
export default function Vendas() {
  ...
  async function carregar() {
    const [{ data: v }, { data: c }, { data: p }] = await Promise.all([
      supabase.from('vendas').select('*, clientes(nome, telefone, endereco, numero, bairro, complemento), produtos(nome, categoria)').order('created_at', { ascending: false }).limit(1000),
